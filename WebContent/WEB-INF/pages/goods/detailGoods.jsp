...
var theurls = new Array();
    ...
    function changeurl(butdom){
    
    var index = butdom.value-1;
    
    $("#goodimg").attr("src",theurls[index]);
    ...
    //获取img对象
		...
		var oldimgurl = img.attr("src");
		
		$("#urltemplete").val(oldimgurl);
		
		var urltemplete = $("#urltemplete").val();
		
		//获取url
		var urls = $("#img_hidden").val().split(";");
		...
		htmlscripts = htmlscripts + "<input type='button' value="+(i+1)+" style='width:20px;height:20px;margin-left:10px;' onclick='changeurl(this)'>";
		
		var newurl = urltemplete.substring(0,oldimgurl.length-1)+urls[i];
		
		theurls[i] = newurl;
		
		img.attr("src",newurl);
		}
		$("#changeimgdiv").html(htmlscripts);
    }
       function showLogin() {
		...
		<nav class="white nav1">
			<div class="nav-wrapper">
				<a href="<%=basePath%>goods/homeGoods" class="logo"> <em
					class="em1">西安航空学院</em> <em class="em2">校园二手市场</em> <em class="em3"></em>
				</a>
				...
				<ul class="right">
					...
					<c:if test="${!empty cur_user}">
						...
						<li class="changemore"><a class="changeMoreVertShow()">设置
								<!-- <i class="iconfont"></i> -->
						</a>
							<div class="more-vert">
...
-->
	...
	<div ng-controller="detailBoxController"
		...
		<div class="col s6">
			<div class="slider"  style="height: 440px;">
				<ul class="slides" style="height: 400px;">
					<img id="goodimg" src="<%=basePath%>upload/}" />
				</ul>
		...
		<div class="col s6">
			...
			<div class="item-public-info">
				...
				<p>
					<i class="iconfont"></i> <em class="item-location">西安航空学院</em>
				</p>
			</div>
	...
	<div class="detail-box stark-components z-depth-1 row">
		...
		<p class="section"></p>
		<p class="section">联系我的时候，请说明是在“西安航空学院校园二手市场”上看见的哦~</p>
	</div>
		...
		<div class="clo s12">
			<div ng-controller="commentController"
				...
				<form id="addCommentForm" class="form-horizontal">
					...
					<input type="hidden" id="img_hidden" value="${urls}"> <input
						type="hidden" id="urltemplete" value="">
				</form>
			</div>
		</div>
	</div>
