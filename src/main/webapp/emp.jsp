	...
	$(function() {
		...
		$.getJSON("doinit_emp.do", function(map) {
			...
			for (var i = 0; i < lswf.length; i++) {
				var wf = lswf[i];
				$("#wf").append(
						"<input type='checkbox' name='wids' value='"+wf.wid+"'/>"
								+ wf.wname);
			}
			/* 部门 */
			$('#depid').combobox({
				...
				.datagrid(
						{
							...
							columns : [ [
									...
									},
									{
										...
										align : 'center',
										formatter : function(value, row, index) {
											var bt1 = '<input type="button" value="删除" onclick="dodelById('
													+ row.eid + ')">';
											var bt2 = '<input type="button" value="编辑" onclick="findById('
													+ row.eid + ')">';
											var bt3 = '<input type="button" value="详细" onclick="findDetail('
													+ row.eid + ')">';
											return bt1 + '&nbsp;' + bt2
													+ '&nbsp;' + bt3;
										}
...
<body>
	...
	<!--加入一个提示窗口，显示详细信息  -->
	<div id="winemp" class="easyui-window" title="员工详细信息"
		...
		<table width="550" border="1" align="center" cellpadding="1"
			...
			<tr>
				...
				<td width="129" rowspan="7"><img id="dtmyphoto"
					src="uppic/default.jpg" width="129" height="150" /></td>
			</tr>
			<tr>
				<td>性别</td>
